package com.evolutionnext.akkastreams
...
import akka.stream.scaladsl.{Broadcast, FileIO, Flow, GraphDSL, Keep, Merge, RunnableGraph, Sink, Source, SourceQueueWithComplete}
...
class SimpleStreamSpec extends FunSuite with Matchers {
  ...
  test(
    """This is the same as the previous example, with the exception that we will use a Materializer that
      | is implicitly bound so as not to litter our code with explicit bindings.""") {
    val single: Source[Int, NotUsed] = Source.single(3)
  ...
  test("""DSL graphs show the flow of data Broadcast and Merge using the Scala DSL""") {
